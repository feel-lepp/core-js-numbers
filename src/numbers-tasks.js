...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lengthOfFirstVector = Math.sqrt(x1 ** 2 + y1 ** 2);
  const lengthOfSecondVector = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosAlpha = dotProduct / (lengthOfFirstVector * lengthOfSecondVector);
...
function roundToPowerOfTen(num, pow) {
  ...
  const reminder = num % powerOfTen;
  const reducedNumber =
    reminder >= powerOfTen / 2 ? num + powerOfTen - reminder : num - reminder;
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    length = numbers.push(numbers[length - 1] + numbers[length - 2]);
  }
  return numbers.at(length - 1);
}
